...
const ProfileCardComponent = ({
  ...
  useEffect(() => {
    ...
    // Track if deviceorientation listener was added
    let deviceOrientationAdded = false;

    const handleClick = () => {
      if (!enableMobileTilt || location.protocol !== 'https:' || deviceOrientationAdded) return;
      if (typeof (DeviceMotionEvent as any).requestPermission === 'function') {
        (DeviceMotionEvent as any).requestPermission()
          .then((state: string) => {
            if (state === 'granted') {
              window.addEventListener('deviceorientation', deviceOrientationHandler);
              deviceOrientationAdded = true;
            }
          })
    ...
    return () => {
      ...
      // Always try to remove deviceorientation listener in cleanup
